...
class FakeEnrollmentRepository:
    ...
    async def get_by_user_and_course(self, user_id: str, course_id: str):
        return next(
            (
                e
...
class FakeCourseRepository:
    """Minimal fake course repository for EnrollUserUseCase."""

    async def get_by_id(self, course_id: str):
        """Return None — course lookup is only needed for re-enrollment."""
        return None
...
class TestEnrollUserUseCase:
    async def test_creates_enrollment_successfully(self):
        ...
        use_case = EnrollUserUseCase(
            repository=repo, course_repository=FakeCourseRepository()
        )
    ...
    async def test_initial_access_type_is_preview(self):
        repo = FakeEnrollmentRepository()
        use_case = EnrollUserUseCase(
            repository=repo, course_repository=FakeCourseRepository()
        )
        ...
        assert result.access_type == AccessType.PREVIEW
